...
<task type="auto">
  ...
  <action>
    ...
    Deno.serve(async (req) => {
      ...
      try {
        ...
        for (const referral of referrals) {
          ...
          if (markError || !markError === false) continue; // If update failed (already rewarded), skip
    ...
    **Critical guards to verify are in the code:**
    ...
    - `.eq('status', 'pending')` on the UPDATE (idempotency — only updates once)
...
- idempotency guard: `.update({ status: 'rewarded' }).eq('status', 'pending')` pattern present
...
- Idempotency: marks referral 'rewarded' before inserting credit — retry-safe
