  ...
  const renderPrice = () => {
    if (plan.tier === 'free') {
      return (
        <div>
          ...
          <p className="text-xs text-ink/80 mt-1">Free forever</p>
        </div>
      );
    }
    ...
    if (interval === 'month') {
      return (
        <div>
          ...
          <span className="text-sm text-ink/80">/mo</span>
        </div>
      );
    }
    ...
    return (
      <div>
        ...
        <span className="text-sm text-ink/80">/mo</span>
        <p className="text-xs text-ink/80 mt-1">billed ${plan.annualPrice.toFixed(2)}/year</p>
      </div>
    );
  };
  ...
  const renderCTA = () => {
    ...
    if (plan.tier === 'free') {
      ...
      if (isAuthenticated && (currentTier === 'free' || currentTier === null)) {
        return (
          <p className="text-[10px] uppercase tracking-[0.2em] text-ink/85 text-center">
            Current Plan
          </p>
        );
      }
      ...
      <ul className="space-y-3">
        {plan.features.map((feature) => (
          <li key={feature} className="flex items-start gap-2 text-sm text-ink/80">
            <Check className="w-4 h-4 text-ink/60 mt-0.5 shrink-0" />
            <span>{feature}</span>
          </li>
        ))}
      </ul>
      <div className="pt-2">
        {isNativeiOS() ? null : renderCTA()}
      </div>
    </div>
  );
};
