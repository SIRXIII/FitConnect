...
const Section: React.FC<{ title: string; subtitle?: string; children: React.ReactNode }> = ({
  ...
  <div className="border border-ink/10 p-8 space-y-6">
    <div className="space-y-1">
      <p className="text-xs uppercase tracking-[0.2em] text-ink/85 font-medium">{title}</p>
      {subtitle && <p className="text-sm text-ink/85 font-light">{subtitle}</p>}
    </div>
    {children}
  </div>
);
...
const Field: React.FC<{ label: string; children: React.ReactNode; hint?: string }> = ({
  ...
  <div className="space-y-2">
    <label className="text-[10px] uppercase tracking-[0.2em] text-ink/85">{label}</label>
    {children}
    {hint && <p className="text-[10px] text-ink/80">{hint}</p>}
  </div>
);
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="Profile Info" subtitle="Visible to clients when they view your trainer card.">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-ink/80">Auto-compressed to save space</p>
          </div>
          <input
            ref={avatarInputRef}
            type="file"
      ...
      >
        ...
        {savingIntroOffer && <p className="text-[10px] text-ink/80">Saving…</p>}
      </Section>
      ...
      >
        {trainerProfile?.id ? (
          ...
          <p className="text-sm text-ink/80 font-light">Loading trainer profile...</p>
        )}
      </Section>
      ...
      <div className="border border-red-200/60 p-8 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm font-light text-ink/85">
