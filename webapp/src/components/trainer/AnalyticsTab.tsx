    ...
    <div className="space-y-10 py-6">
      ...
      {metrics ? (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="border border-ink/10 p-6 space-y-1">
            <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Gross Earnings</p>
            <p className="text-2xl serif font-light text-ink">${metrics.gross_earnings.toFixed(2)}</p>
          </div>
          <div className="border border-ink/10 p-6 space-y-1">
            <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Net Earnings</p>
            <p className="text-2xl serif font-light text-ink">${metrics.net_earnings.toFixed(2)}</p>
          </div>
          <div className="border border-ink/10 p-6 space-y-1">
            <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Bookings</p>
            <p className="text-2xl serif font-light text-ink">{metrics.booking_count}</p>
          </div>
          <div className="border border-ink/10 p-6 space-y-1">
            <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Avg Price</p>
            <p className="text-2xl serif font-light text-ink">${metrics.avg_price.toFixed(2)}</p>
          </div>
          <div className="border border-ink/10 p-6 space-y-1">
            <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Discount Adoption</p>
            <p className="text-2xl serif font-light text-ink">{metrics.discount_adoption_pct}%</p>
            <p className="text-[8px] text-ink/80">bookings with active discount</p>
          </div>
        </div>
      ) : (
        <div className="border border-ink/10 p-8 text-center">
          <p className="text-sm text-ink/80">No bookings in this period.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="border border-ink/10 p-6 space-y-3">
          <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Revenue Trend</p>
        ...
        {/* Booking Count */}
        <div className="border border-ink/10 p-6 space-y-3">
          <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Booking Count</p>
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="border border-ink/10 p-6 space-y-4">
        <p className="text-[9px] uppercase tracking-[0.2em] text-ink/85 font-medium">Peak Booking Hours</p>
        ...
        <div className="overflow-x-auto">
          <div className="flex gap-2" style={{ minWidth: 600 }}>
            ...
            <div className="flex flex-col gap-[2px] justify-around" style={{ minWidth: 28 }}>
              {DAYS.map((day) => (
                <span key={day} className="text-[8px] text-ink/80 text-right pr-1 leading-none" style={{ height: 16, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                  {day}
                </span>
              ))}
            </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 border border-ink/20 text-xs uppercase tracking-[0.15em] text-ink/80 hover:text-ink hover:border-ink/40 transition-colors"
        >
