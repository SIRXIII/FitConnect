const COOKIE_NAME = 'fitc_ref';
const COOKIE_DAYS = 30;

export function captureReferralCode(code: string): void {
  // First-touch: the trainer who referred this visitor first keeps the
  // attribution. A later ?ref= from a different trainer must not overwrite it.
  // This matches the first-touch UTM capture in UrlParamCapture.
  // Storage access is guarded: this runs in an effect on every route and
  // document.cookie can throw in cookie-blocked in-app webviews (IG/TikTok).
  try {
    if (readReferralCode()) return;
    const expires = new Date(Date.now() + COOKIE_DAYS * 86400 * 1000).toUTCString();
    document.cookie = `${COOKIE_NAME}=${encodeURIComponent(code)}; expires=${expires}; path=/; SameSite=Lax`;
  } catch {
    // Cookies unavailable; drop attribution rather than crash the app.
  }
}

export function readReferralCode(): string | null {
  try {
    const match = document.cookie.match(new RegExp(`(?:^|; )${COOKIE_NAME}=([^;]*)`));
    return match ? decodeURIComponent(match[1]) : null;
  } catch {
    return null;
  }
}

export function clearReferralCode(): void {
  document.cookie = `${COOKIE_NAME}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Lax`;
}

export function buildReferralLink(code: string): string {
  return `${window.location.origin}/?ref=${encodeURIComponent(code)}`;
}

export function buildTrainerReferralLink(code: string, trainerPath: string): string {
  return `${window.location.origin}${trainerPath}?ref=${encodeURIComponent(code)}`;
}
