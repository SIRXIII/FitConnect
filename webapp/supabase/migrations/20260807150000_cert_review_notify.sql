-- admin_review_cert: notify the trainer when a cert is rejected or needs more info.
-- Body rebuilt from live pg_get_functiondef (2026-08-07) so this does not revert live-only fixes.
-- The notify block is wrapped so a notification failure can never roll back the review decision.
