# FitRush Project — Codex Context

## App Identity
- **App Name:** FitRush (originally FitConnect — repo/folder still named FitConnect)
- **Live URL:** https://fitrush.io (Cloudflare Pages project `fitconnect`)
...
- **Hosting:** Cloudflare Pages (frontend — project `fitconnect`, domain fitrush.io), Supabase (backend + edge functions). NOTE: the Netlify site `fitrush-app` has NO custom domain and is NOT the deploy target — a green Netlify build proves nothing.
...
- Uses **/gsd** (Get Shit Done) Codex workflow for structured, phase-based development
